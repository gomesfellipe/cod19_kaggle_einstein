...
A descrição completa do problema pode ser obtida no repositório oficial do projeto. A finalidade deste repositório é resolver a [Task 2](https://www.kaggle.com/einsteindata4u/covid19/tasks?taskId=646) proposta para este dataset, que consistem em: 
...
O objetivo final deste projeto será: Prever se os casos confirmados de COVID-19 serão internados.
...
create_report(janitor::clean_names(dataset), y = 'patient_addmited_to_all', 
...
dataset_initial_split  <- initial_split(dataset, 
                                        strata = patient_addmited_to_all,
                                        prop = 0.9)

recipe_ini <- 
  recipe(patient_addmited_to_all ~ ., data = training(dataset_initial_split)) %>%
...
```{r}
table(train_ini$patient_addmited_to_all)
```
...
```{r}
train_ini_down <- 
  downSample(y = train_ini %>% pull(patient_addmited_to_all),
             x = train_ini %>% select(-patient_addmited_to_all),
             yname = "patient_addmited_to_all") %>% 
  as_tibble()

train_knn_down <- 
  downSample(y = train_knn %>% pull(patient_addmited_to_all),
             x = train_knn %>% select(-patient_addmited_to_all),
             yname = "patient_addmited_to_all") %>% 
  as_tibble()

train_ini_smote <- 
  SMOTE(patient_addmited_to_all ~ ., data  = as.data.frame(train_ini)) %>% 
  as_tibble()

train_knn_smote <- 
  SMOTE(patient_addmited_to_all ~ ., data  = as.data.frame(train_knn)) %>% 
  as_tibble()
```
...
custom_rfe <- function(x){
  rfe(x %>% select(-patient_addmited_to_all),
      x %>% pull(patient_addmited_to_all),
...
to_roc <- 
  tibble(
    original = testing(dataset_initial_split) %>% 
      pull(patient_addmited_to_all) %>% {if_else(. == "negative", 0, 1)}
    ,
...
```{r}
confusionMatrix(
  predict(rfe_knn, newdata = test_knn) %>% pull(pred),
  test_knn %>% pull(patient_addmited_to_all)
)
```
...
O modelo final selecionando será o 1 (RFE com input knn sem balanceamento). Veja um gráfico que exibe o melhor k para ajustar o modelo:
...
xgb_model <- train(patient_addmited_to_all ~ .,
                   train_knn_smote,
                   method = 'gbm',
                   tuneLength = 10,
                   # tuneGrid = grid,
